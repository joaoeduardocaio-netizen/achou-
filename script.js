document.addEventListener("DOMContentLoaded", () => {
  ...
  async function searchProducts(
    ...
    try {
      const response =
        await fetch(
          `${CONFIG.api}?q=${encodeURIComponent(
            query
          )}`,
          {
            headers: {
  ...
  document
    ...
    ?.addEventListener(
      ...
      () => {
        alert(
          "O ACHOU! direciona você para a loja parceira. A compra é finalizada no parceiro."
        );
      }
    );
      ...
      button.addEventListener(
        ...
        () => {
          ...
          if (
            ...
            alert(
              "A compra é concluída diretamente na loja parceira."
            );
          }
        }
      );
